...
public class YouTubeSubtitleService implements SubtitleService {
    ...
    public List<SubtitleLine> fetch(String videoId)
        ...
        boolean englishTrackSeen = false;

        // ---- Tier 0: NewPipeExtractor -------------------------------------
        try {
            NewPipeResult result = fetchViaNewPipe(videoId);
            Log.d(TAG, "tier0 newpipe lines=" + result.lines.size()
                    + " englishTrack=" + result.englishTrackFound);
            if (!result.lines.isEmpty()) return result.lines;
            // Only treat as "track exists" if NewPipe found an English track.
            // Videos that only have e.g. Vietnamese / Spanish caption tracks
            // must be reported as Unavailable, not FetchFailed.
            englishTrackSeen |= result.englishTrackFound;
        } catch (Exception e) {
        ...
        if (englishTrackSeen) {
            throw new FetchFailedException(
                    "English caption track exists but body could not be downloaded for "
                            + videoId, null);
        }
    ...
    /** Outcome of one NewPipe pass. Carries enough info for the caller to
     *  distinguish "no English track at all" (→ Unavailable) from
     *  "English track found but body unparseable" (→ FetchFailed). */
    private static final class NewPipeResult {
        final List<SubtitleLine> lines;
        final boolean englishTrackFound;

        NewPipeResult(List<SubtitleLine> lines, boolean englishTrackFound) {
            this.lines = lines;
            this.englishTrackFound = englishTrackFound;
        }

        static NewPipeResult empty(boolean englishTrackFound) {
            return new NewPipeResult(Collections.emptyList(), englishTrackFound);
        }
    }

    @NonNull
    private NewPipeResult fetchViaNewPipe(String videoId) throws Exception {
        ...
        if (tracks == null || tracks.isEmpty()) {
            ...
            return NewPipeResult.empty(false);
        }

        SubtitlesStream pick = pickEnglish(tracks);
        if (pick == null) {
            // Video has caption tracks but none in English (e.g. only VI/ES/JA).
            // Treat as "no English available" so caller surfaces Unavailable.
            Log.d(TAG, "tier0 newpipe no English track among " + tracks.size()
                    + " tracks (langs=" + listLangs(tracks) + ")");
            return NewPipeResult.empty(false);
        }
        ...
        if (body == null || body.isEmpty()) {
            // English track exists but download failed.
            return NewPipeResult.empty(true);
        }
        ...
        return new NewPipeResult(parsed, true);
    }

    private static String listLangs(@NonNull List<SubtitlesStream> tracks) {
        StringBuilder sb = new StringBuilder();
        for (SubtitlesStream s : tracks) {
            if (sb.length() > 0) sb.append(',');
            sb.append(s.getLanguageTag());
        }
        return sb.toString();
    }
