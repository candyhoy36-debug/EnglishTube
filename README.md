...
- Player tự xây: WebView phát `m.youtube.com/watch?v=...` (bypass embed restrictions) + FAB mở bottom sheet phụ đề song ngữ tự cuộn theo thời gian.
...
- WebView (Android system) + JS bridge `WebViewPlayerBridge` để đồng bộ `currentTime`.
...
| Sprint | Nội dung |
|---|---|
| 0 | Skeleton: Gradle, AndroidX, Room, OkHttp, ML Kit, package layout. App build & launch một MainActivity placeholder. |
...
| 2 | WebView player + JS bridge currentTime + FAB bottom-sheet phụ đề EN + auto-scroll + xử lý case không có phụ đề. |
